...
def get_aura_history(
    ...
    while current_dt <= end_dt:
        ...
        try:
            ...
            logger.info(f"[aura] Error on {current_dt.date()} for {token_symbol}: {e}")
    ...
    if output:
        logger.protocol_end("aura", token_symbol, output)


def process_all_aura_tokens(
    chain: str,
    ...
    for address, info in tokens.items():
        ...
        logger.protocol_start("aura", symbol, resolved_start_date, end_date)
        get_aura_history(
            chain=chain,
